...
function isString(value) {
  let res = false;
  if (typeof value === 'string' || value instanceof String) res = true;
  return res;
}
...
function repeatString(str, times) {
  let res = '';
  if (times > 0) res = str.repeat(times);
  return res;
}
...
function removeFirstOccurrences(str, value) {
  let res = '';
  if (str.indexOf(value) !== -1) res = str.replace(value, '');
  else res = str;
  return res;
}
...
function removeLastOccurrences(str, value) {
  ...
  let end = str.slice(str.lastIndexOf(value));
  end = end.replace(value, '');
  return start + end;
}
...
function formatTime(minutes, seconds) {
  const min = minutes < 10 ? String(minutes).padStart(2, 0) : String(minutes);
  const sec = seconds < 10 ? String(seconds).padStart(2, 0) : String(seconds);
  const res = `${min}:${sec}`;
  return res;
}
...
function countVowels(str) {
  ...
  const vow = 'aeiouy';
  let count = 0;
  for (let i = 0; i < s.length; i += 1) if (vow.includes(s[i])) count += 1;
  return count;
}
...
function isPalindrome(str) {
  ...
  strNew = strNew.split(' ').join('').toLocaleLowerCase();
...
function findLongestWord(sentence) {
  ...
  if (sentence.length > 0) {
    ...
    for (let i = 0; i < arr.length; i += 1) {
      if (i === 0) word = arr[i];
      if (i > 0 && arr[i].length > arr[i - 1].length) word = arr[i];
    }
  }
  return word;
}
...
function invertCase(str) {
  const res = str.split('').map((s) => {
    let sNew = s;
    if (s === s.toLowerCase()) sNew = s.toUpperCase();
...
function encodeToRot13(str) {
  ...
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    if (normal.indexOf(str[i]) !== -1) res += code[normal.indexOf(str[i])];
    else res += str[i];
  }
  return res;
}
